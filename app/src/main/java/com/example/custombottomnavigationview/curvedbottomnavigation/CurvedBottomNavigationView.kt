...
import com.example.custombottomnavigationview.R
...
class CurvedBottomNavigationView @JvmOverloads constructor(
    ...
    private lateinit var textviews: Array<BottomNavTextViewItem>
    ...
    fun setMenuItems(cbnMenuItems: Array<CbnMenuItem>, activeIndex: Int = 0) {
        ...
        textviews = Array(cbnMenuItems.size) {
            BottomNavTextViewItem(context)
        }
        initializeMenuIcons()
        initializeMenuAVDs()
    ...
    private fun initializeBottomItems(activeItem: Int) {
        ...
        menuIcons.forEachIndexed { index, icon ->
            ...
            val textviewItem = textviews[index]
            textviewItem.setTextView(cbnMenuItems[index].title)
            ...
            innerLinearLayout.addView(menuItem, layoutParams)

            // adding textview to layout

            textView.text = cbnMenuItems[index].title
            ...
            if (index == activeItem) {
                // set textview visibility invisible
                textviews[index].textSize = 13f
                textviews[index].typeface = Typeface.DEFAULT_BOLD

            }
            innerLinearLayout.addView(textviewItem, layoutParams)
            val innerLinearLayoutParams =
                LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT)
            innerLinearLayoutParams.weight = 1f
            bottomNavLayout.addView(innerLinearLayout, innerLinearLayoutParams)
    ...
    fun onMenuItemClick(index: Int) {
        ...
        textviews.forEachIndexed { index, bottomNavTextViewItem ->
            if (selectedIndex == index){
                textviews[index].textSize = 13f
                textviews[index].typeface = Typeface.DEFAULT_BOLD

            }else{
                textviews[index].textSize = 11f
            }
        }
